...
class FDL(nn.Module):
    ...
    Handles misaligned training data by operating in frequency space
    '''
    ...
    def forward(self, pred, target):
        ...
        pred_amp, pred_phase = torch.abs(pred_freq), torch.angle(pred_freq)
        target_amp, target_phase = torch.abs(target_freq), torch.angle(target_freq)
        
        # Distribution matching
        amp_loss = nn.L1Loss()(pred_amp, target_amp)
...
class PerceptualLoss(nn.Module):
    def __init__(self):
        super().__init__()
        vgg = vgg19(weights= 'DEFAULT').features[:35].eval()
    ...
    def forward(self, output, target):
        ...
        vgg_tgt = self.vgg(target.detach())
        return self.loss(vgg_out, vgg_tgt)
    
class LearnablePoissonGaussianLoss(nn.Module):
    '''
    To make the Poisson-Gaussian loss learnable with respect to parameters 'a' and 
    'b', you must define 'a' and 'b' as nn.Parameter so they are optimized alongside the model weights.
    Ensure they remain positive (since they represent variance components) by parameterizing them 
    through a softplus or exponential transformation.

    Image formation model: \\sigma^2=a\\mu+b
    where \\sigma^2= combined noise model variance, 
    a= Poisson noise scaling factor
    b= Gaussian noise variance

    Inputs: 
    a_init=1.0, # initial value of 'a'
    b_init=0.01, # initial value of 'b'
    eps=1e-6 # To avoid numerical stability

    Usage:
        Instantiate the loss:
        loss_fn = LearnablePoissonGaussianLoss()

        Add the loss parameters to your optimizer:
            optimizer = torch.optim.Adam(list(model.parameters()) + list(loss_fn.parameters()), lr=1e-3)
    '''
    def __init__(self, a_init=1.0, b_init=0.01, eps=1e-6):
        super().__init__()
        # Use softplus to ensure positivity
        self.a_param = nn.Parameter(torch.tensor([a_init]).log().exp())  # log-exp for stability
        self.b_param = nn.Parameter(torch.tensor([b_init]).log().exp())
        self.eps = eps

    def forward(self, mu, target):
        # Ensure a and b are positive
        a = F.softplus(self.a_param)
        b = F.softplus(self.b_param)
        var = a * mu + b
        var = var + self.eps  # Numerical stability
        loss = (target - mu) ** 2 / var + torch.log(var)
        return loss.mean()
...
class GANLoss(nn.Module):
    ...
    def forward(self, pred_real, pred_fake=None, is_real=True):
        if self.relativistic and pred_fake is not None:
            return self.loss(pred_real - pred_fake.mean(0, keepdim=True), 
                            torch.ones_like(pred_real) if is_real else torch.zeros_like(pred_real))
        return self.loss(pred_real, torch.ones_like(pred_real) if is_real else torch.zeros_like(pred_real))
