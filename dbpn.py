...
from attention_networks import ExternalAttention2D
...
class AttentionNet(nn.Module):
    def __init__(self, num_channels, base_filter, feat, memory_units, num_stages, scale_factor):
        ...
        self.up1 = UpBlock(base_filter, kernel, stride, padding)
        self.attention1 = ExternalAttention2D(in_channels=base_filter, S=memory_units)
        self.down1 = DownBlock(base_filter, kernel, stride, padding)
        self.up2 = UpBlock(base_filter, kernel, stride, padding)
        self.attention2 = ExternalAttention2D(base_filter, memory_units)
        self.down2 = D_DownBlock(base_filter, kernel, stride, padding, 2)
        self.up3 = D_UpBlock(base_filter, kernel, stride, padding, 2)
        self.attention3 = ExternalAttention2D(base_filter, memory_units)
        self.down3 = D_DownBlock(base_filter, kernel, stride, padding, 3)
        self.up4 = D_UpBlock(base_filter, kernel, stride, padding, 3)
        self.attention4 = ExternalAttention2D(base_filter, memory_units)
        self.down4 = D_DownBlock(base_filter, kernel, stride, padding, 4)
        self.up5 = D_UpBlock(base_filter, kernel, stride, padding, 4)
        self.attention5 = ExternalAttention2D(base_filter, memory_units)
        self.down5 = D_DownBlock(base_filter, kernel, stride, padding, 5)
        self.up6 = D_UpBlock(base_filter, kernel, stride, padding, 5)
        self.attention6 = ExternalAttention2D(base_filter, memory_units)
        self.down6 = D_DownBlock(base_filter, kernel, stride, padding, 6)
        self.up7 = D_UpBlock(base_filter, kernel, stride, padding, 6)
        self.attention7 = ExternalAttention2D(base_filter, memory_units)
        #Reconstruction
        self.output_conv = ConvBlock(num_stages*base_filter, num_channels, 3, 1, 1, activation=None, norm=None)
    ...
    def forward(self, x):
        ...
        h1 = self.up1(x)
        ea1=self.attention1(h1)
        l1 = self.down1(ea1)
        h2 = self.up2(l1)
        ea2=self.attention2(h2)
        concat_h = torch.cat((ea1, ea2),1)
        l = self.down2(concat_h)
        
        concat_l = torch.cat((l, l1),1)
        h = self.up3(concat_l)
        ea3=self.attention3(h)
        concat_h = torch.cat((ea3, concat_h),1)
        l = self.down3(concat_h)
        
        concat_l = torch.cat((l, concat_l),1)
        h = self.up4(concat_l)
        ea4=self.attention4(h)
        concat_h = torch.cat((ea4, concat_h),1)
        l = self.down4(concat_h)
        
        concat_l = torch.cat((l, concat_l),1)
        h = self.up5(concat_l)
        ea5=self.attention3(h)
        concat_h = torch.cat((ea5, concat_h),1)
        l = self.down5(concat_h)
        
        concat_l = torch.cat((l, concat_l),1)
        h = self.up6(concat_l)
        ea6=self.attention3(h)
        concat_h = torch.cat((ea6, concat_h),1)
        l = self.down6(concat_h)
        
        concat_l = torch.cat((l, concat_l),1)
        h = self.up7(concat_l)
        ea7=self.attention3(h)
        concat_h = torch.cat((ea7, concat_h),1)
        x = self.output_conv(concat_h)
