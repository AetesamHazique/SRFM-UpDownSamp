...
Specimen =  'ER';% 'F-actin';  | ER | Microtubules | F-actin | F-actin_Nonlinear

SavePath = ['H:\Image Database\Real Fluores Mic\BioSR\Training/' Specimen]; 
DataPath = ['H:\Image Database\Real Fluores Mic\BioSR/' Specimen];
...
RotFlag = 0;     % 0 for no rotation, 1 for random rotation, 2 for flipping and random rotaion
SaveRawFlag = 1; % 0 for only saving WF patches, 1 for saving raw SIM image patches

SegX = 32; % target patch size x
SegY = 32; % target patch size y
ValidateNum = 1000;  % number of patch pairs for validation
TotalSegNum = 50000; % number of patch pairs for training
...
if contains(SavePath,'Nonlinear') 
    n_perSIM = 25; 
else
    n_perSIM = 9; 
end
...
if SaveRawFlag == 1
    if exist(save_training_path,'dir') 
        rmdir(save_training_path,'s'); 
    end
    if exist(save_validate_path,'dir')
        rmdir(save_validate_path,'s'); 
    end
    mkdir(save_training_path);
    mkdir(save_validate_path);
end
if exist(save_training_WF_path,'dir') 
    rmdir(save_training_WF_path,'s'); 
end
if exist(save_validate_WF_path,'dir')
    rmdir(save_validate_WF_path,'s'); 
end
if exist(save_training_gt_path,'dir')
    rmdir(save_training_gt_path,'s'); 
end
if exist(save_validate_gt_path,'dir')
    rmdir(save_validate_gt_path,'s'); 
end
mkdir(save_training_gt_path);
mkdir(save_training_WF_path);
mkdir(save_validate_gt_path);
mkdir(save_validate_WF_path);
